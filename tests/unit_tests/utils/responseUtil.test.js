...
describe('ResponseUtil', () => {
  ...
  describe('buildSuccessResponse', () => {
    ...
    it(`should return response with statusCode ${statusCode}`, async () => {
      const res = responseUtil.buildSuccessfulResponse(data)
      expect(res.statusCode).equal(200)
      expect(res.body).deep.equal(JSON.stringify(data))
    })
    it(`should return response with statusCode ${statusCode} with no data`, async () => {
  ...
  describe('buildErrorResponse', () => {
    ...
    _.forEach(mockResponses, (mockResponse) => {
      it(`should return response with statusCode ${mockResponse.statusCode} and body contains ${mockResponse.message}`, async () => {
